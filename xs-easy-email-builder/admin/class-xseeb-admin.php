...
class XSEEB_Admin {
	...
	public function enqueue_styles() {
		...
		if ( isset( $_GET['page'] ) && 'xseeb-page' === $_GET['page'] ) {
			wp_enqueue_style( 'xseeb-react', XSEEB_PLUGIN_URL . 'build/css/style.css', array(), $this->version, 'all' );
			//phpcs:ignore
			if ( isset( $_GET['id'] ) ) {
		...
		if ( isset( $_GET['page'] ) && 'xseeb-support' === $_GET['page'] ) {
			wp_enqueue_style( 'bootstrap', XSEEB_PLUGIN_URL . 'admin/css/bootstrap.min.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		//phpcs:ignore
		if ( isset( $_GET['page'] ) && 'xseeb-page' === $_GET['page'] ) {
			$plugin_url  = XSEEB_PLUGIN_URL . 'build/';
			...
			wp_localize_script(
				...
				array(
					...
					'xseeb_plugin_url' => XSEEB_PLUGIN_URL . 'build/',
					'nonce'            => wp_create_nonce( 'wp_rest' ),
				)
			);
			// Footer + dependency so localized `xseeb_base_url` exists before ES modules run (see axios.config.ts).
			wp_enqueue_script( 'xseeb-react-vendor', XSEEB_PLUGIN_URL . 'build/js/vendor.js', array( $this->easy_email_builder ), $this->version, true );
			wp_enqueue_script( 'xseeb-react', XSEEB_PLUGIN_URL . 'build/js/index.js', array( $this->easy_email_builder, 'xseeb-react-vendor' ), $this->version, true );

		}
		//phpcs:ignore
		if ( isset( $_GET['page'] ) && 'xseeb-support' === $_GET['page'] ) {
			wp_enqueue_script( 'bootstrap', XSEEB_PLUGIN_URL . 'admin/js/bootstrap.min.js', array(), $this->version, true );
	...
	public function xseeb_add_menu() {
		...
		add_submenu_page(
			'xseeb-page',
			esc_html__( 'Support', 'xs-easy-email-builder' ),
			esc_html__( 'Support', 'xs-easy-email-builder' ),
			'manage_options',
			'xseeb-support',
			array( $this, 'xseeb_support_page' )
		);
	}
		/**

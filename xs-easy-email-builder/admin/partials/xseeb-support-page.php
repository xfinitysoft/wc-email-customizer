...
$xseeb_selected_tab = 'report';
if ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'xseeb-setting' ) ) {
	if ( isset( $_GET['tab'] ) && ! empty( $_GET['tab'] ) ) {
		$xseeb_selected_tab = sanitize_text_field( wp_unslash( $_GET['tab'] ) );
	}
}
$allowed_tabs = array( 'report', 'request', 'hire', 'review' );
if ( ! in_array( $xseeb_selected_tab, $allowed_tabs, true ) ) {
	$xseeb_selected_tab = 'report';
}
?>
<div class="warp">
	...
	<nav class="nav-tab-wrapper wp-clearfix" aria-label="Secondary menu">
		<a class="nav-tab <?php echo ( 'report' === $xseeb_selected_tab ) ? 'nav-tab-active' : ''; ?>" href="<?php echo esc_url( wp_nonce_url( '?page=xseeb-support&tab=report', 'xseeb-setting' ) ); ?>">
			<?php esc_html_e( 'Report a bug', 'xs-easy-email-builder' ); ?>
		</a>
		<a class="nav-tab <?php echo ( 'request' === $xseeb_selected_tab ) ? 'nav-tab-active' : ''; ?>" href="<?php echo esc_url( wp_nonce_url( '?page=xseeb-support&tab=request', 'xseeb-setting' ) ); ?>">
			<?php esc_html_e( 'Request a Feature', 'xs-easy-email-builder' ); ?>
		</a>
		<a class="nav-tab <?php echo ( 'hire' === $xseeb_selected_tab ) ? 'nav-tab-active' : ''; ?>" href="<?php echo esc_url( wp_nonce_url( '?page=xseeb-support&tab=hire', 'xseeb-setting' ) ); ?>">
			<?php esc_html_e( 'Hire US', 'xs-easy-email-builder' ); ?>
		</a>
		<a class="nav-tab <?php echo ( 'review' === $xseeb_selected_tab ) ? 'nav-tab-active' : ''; ?>" href="<?php echo esc_url( wp_nonce_url( '?page=xseeb-support&tab=review', 'xseeb-setting' ) ); ?>">
			<?php esc_html_e( 'Review', 'xs-easy-email-builder' ); ?>
		</a>
	</nav>
	<div class="tab-content">
		<?php
		switch ( $xseeb_selected_tab ) {
			case 'report':
				?>
